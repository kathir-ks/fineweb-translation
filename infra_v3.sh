...
while [ $i -lt $((l + 1)) ]; do
    ...
    output=$(gcloud compute tpus tpu-vm describe "main-$node_id" --zone=europe-west4-a 2>&1)

    while [[ $output != *"READY"* ]]; do
        gcloud compute tpus tpu-vm create "main-$node_id" --zone=europe-west4-a --accelerator-type=v3-8 --version=tpu-ubuntu2204-base --preemptible
        gcloud compute tpus tpu-vm ssh "main-$node_id" --zone=europe-west4-a --command='
