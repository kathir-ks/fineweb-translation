...
if [ -z "$node_id" ] || [ -z "$subset" ] || [ -z "$region" ] || [ -z "$accelerator_type" ] || [ -z "$total_nodes" ] || [ -z "$bucket" ] || [ -z "$dataset" ]; then
    echo "Usage: $0 --node_id NODE_ID --subset SUBSET --region REGION --accelerator_type ACCELERATOR_TYPE --total_nodes TOTAL_NODES --bucket BUCKET --dataset DATASET"
    exit 1
fi
...
while [ $i -lt $((l + 1)) ]; do
    ...
    gcloud compute tpus tpu-vm ssh "main-$node_id" --zone=$region --command="
        cd fineweb-translation;
        python3 inference.py --name $dataset --subset $subset --batch_size 1024 --bucket $bucket --node_id $i --total_nodes $total_nodes"
